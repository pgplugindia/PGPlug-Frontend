...
export default function InterfacePage() {
  ...
  const debouncedSearchTerm = useDebounce(searchParams.location, 500); // 500ms delay
  ...
  useEffect(() => {
    ...
    if (keywordFromUrl) {
      setSearchParams({ location: keywordFromUrl });
    }
  }, []);
  ...
  useEffect(() => {
    const search = async () => {
      if (debouncedSearchTerm.trim()) {
        setIsSearching(true);
        await performKeywordSearch(debouncedSearchTerm);
  ...
  const performKeywordSearch = async (keyword) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/pg/search-location?keyword=${encodeURIComponent(keyword)}`);
              ...
              <div className="w-full bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-1">
                <form onSubmit={handleSearch} className="flex items-center gap-2">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    disabled={isSearching || !searchParams.location.trim()}
                  >
                    {isSearching ? (
